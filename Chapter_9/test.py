...
def make_plot(X, y, plot_name, file_name, XX=None, YY=None, preds=None, dark=False, output_dir=OUTPUT_DIR):
    ...
    if XX is not None and YY is not None and preds is not None:
        plt.contourf(XX, YY, tf.reshape(preds,XX.shape), 25, alpha=0.08, cmap=plt.cm.Spectral)
        plt.contour(XX, YY, tf.reshape(preds,XX.shape), levels=[.5], cmap="Greys", vmin=0, vmax=.6)
...
def dropout_influence(X_train, y_train):
    ...
    for n in range(5):
        ...
        # 生成 x-y 平面采样网格点，方便可视化
        XX, YY = np.meshgrid(xx, yy)
        preds = model(np.c_[XX.ravel(), YY.ravel()])
